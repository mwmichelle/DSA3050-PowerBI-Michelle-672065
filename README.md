...
2. Dimensional modelling using a star schema
3. Business calculations using DAX
4. Interactive dashboard development and business insight generation
...
1. How many bookings and guests are represented in the dataset?
2. What proportion of bookings are cancelled?
3. Which hotel type experiences greater cancellation risk?
4. Does cancellation behaviour change with booking lead time?
5. Which customer types have the highest cancellation rates?
6. Which distribution channels generate higher or lower cancellation risk?
7. What is the average daily rate (ADR)?
8. How long do guests typically stay?
9. Which market segments contribute most strongly to booking activity and estimated realized revenue?
10. How does estimated realized revenue change over time?
11. How does room pricing vary across reserved room types?
...
Each dimension table is connected to `FactBookings` using a **One-to-Many (1:*) relationship**.

-   The dimension table resides on the **1 side**.
    
-   `FactBookings` resides on the **many (*) side**.
...
-   avoids unnecessary bidirectional filtering; and
...
> **Note:** Because the dataset does not contain complete coverage for every calendar year, year-over-year results must be interpreted within the date context being compared.
...
> **Important:** These measures are analytical estimates rather than accounting measures. The dataset does not provide evidence that cancelled inventory was never subsequently resold.

For this reason, the project deliberately uses the terminology **Estimated Cancelled Booking Value** rather than claiming the amount represents proven lost revenue.
...
Approximately **37.08% of bookings were cancelled**.

This means cancellation management represents a significant operational and commercial issue.

### 18.2 City Hotel Experiences Greater Cancellation Risk
...
The result demonstrates a strong **association** between longer lead times and cancellation risk. It does not establish that longer lead time directly causes cancellation.
...
The overall average stay is **3.43 nights**.
...
> The **16.73M** figure should not be interpreted as confirmed lost revenue because rooms from cancelled bookings may have subsequently been resold.
...
-   carefully designed cancellation conditions; and
...
-   unclassified booking sources; or
    
-   a genuine booking channel.
...
│   ├── booking analysis pg 2.png
│   └── revenue and pricing pg 3.png
│
├── model view/
│   └── model view.png
│
└── power query/
    └── staging table applied steps.png
...
![Executive Overview](https://raw.githubusercontent.com/mwmichelle/Hotel-Bookings-DSA3050-EndSem/main/dashboard%20screenshots/executive%20overview%20pg1.png)
...
![Booking Analysis](https://raw.githubusercontent.com/mwmichelle/Hotel-Bookings-DSA3050-EndSem/main/dashboard%20screenshots/booking%20analysis%20pg%202.png)
...
![Revenue Analysis](https://raw.githubusercontent.com/mwmichelle/Hotel-Bookings-DSA3050-EndSem/main/dashboard%20screenshots/revenue%20and%20pricing%20pg%203.png)
...
![Model View](https://raw.githubusercontent.com/mwmichelle/Hotel-Bookings-DSA3050-EndSem/main/model%20view/model%20view.png)
...
![Power Query Transformation Evidence](https://raw.githubusercontent.com/mwmichelle/Hotel-Bookings-DSA3050-EndSem/main/power%20query/staging%20table%20applied%20steps.png)
...
The analysis identifies **cancellation management as a major opportunity**, with an overall cancellation rate of approximately **37.08%**.

Cancellation behaviour varies substantially across hotel types, customer types, distribution channels and, most notably, booking lead time. City Hotel records a cancellation rate of approximately **41.8%**, compared with approximately **27.8%** for Resort Hotel, while bookings made more than one year in advance record particularly high cancellation rates.

At the same time, the commercial dashboard demonstrates the importance of considering **booking value alongside booking volume**. The analysis estimates **42.71M** in total booking value, of which approximately **25.99M** is associated with non-cancelled bookings.

Overall, the project demonstrates how Power BI can transform raw hotel booking data into an interactive analytical solution that supports management in understanding booking behaviour, identifying cancellation risk, evaluating commercial exposure, and making more informed operational decisions.
